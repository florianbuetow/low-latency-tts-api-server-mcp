"""Architecture import rule tests for low-latency-tts-service-mcp.
...
Example rules to add as your project develops layers:
    ...
    def test_services_must_not_import_fastapi(evaluable):
        (
            Rule()
            .modules_that()
            .are_sub_modules_of("low_latency_tts_service_mcp.services")
            .should_not()
            .import_modules_that()
    ...
    def test_config_must_not_import_services(evaluable):
        (
            Rule()
            .modules_that()
            .are_named("low_latency_tts_service_mcp.config")
            .should_not()
            .import_modules_that()
            .are_sub_modules_of("low_latency_tts_service_mcp.services")
            .assert_applies(evaluable)
...
def test_evaluable_is_configured(evaluable: EvaluableArchitecture) -> None:
    ...
    Replace this with a proper canary test once the project has real imports:
        ...
        (
            Rule()
            .modules_that()
            .are_named("low_latency_tts_service_mcp.app")
            .should()
            .import_modules_that()
            .are_named("low_latency_tts_service_mcp.config")
            .assert_applies(evaluable)
